...
| ③ 성과 | "**YTD · 일별**" / "연초 대비 **+8.2%**" (교대) + `chevron.up` | 일별/월별 전환 (현재 단위 라벨, 전환형) | 기간 선택 시트 | 기간·단위 전환 [PM-2] |
...
- 포트폴리오: **종목 추가가 액세서리에서 빠져 목록 화면 우상단 툴바 아이콘 1개로 옮겨간다**
  ...
  화면을 두 겹으로 쌓을 수 있다.
- 성과: 벤치마크 칩 4개를 폐기하고 **선택과 비교 on/off 를 툴바 버튼 하나 + 시트로** 내린다.
  액세서리가 맡는 건 대신 **고빈도인 기간·단위**다 — 배치를 뒤집은 근거는 §4.3 에 있다.
...
| ③ 성과 | 기간·단위 문장 + `chevron.up` + 단위 전환 캡슐 | "YTD" + `chevron.up` + 단위 전환(아이콘 원형) |
...
> 성과 탭의 두 타깃(leading 시트 열기 / trailing 단위 전환)은 히트 프레임이 겹치지 않게
> 분리한다. VoiceOver 에서 "기간과 단위, 버튼" + "표시 단위, 현재 일별, 버튼" 두 요소로
> 읽힌다. 단위 전환은 누를 때마다 대상이 바뀌므로 `.isSelected` 를 달지 않는다.
...
| 2 | 자산 추이 라인 차트 | TrendLineChart — area gradient(brand 계열), 스크럽 지원 [PM-2]. 플롯 높이 240pt. 0% 기준선 + Y축 라벨 2개 · X축 라벨 3개. **벤치마크 범례는 차트 카드 안**, 플롯 위 한 줄(카테고리 dot + 지수 이름) |
| 2-1 | 스크럽 가이드 한 줄 | "차트를 좌우로 쓸어 시점별 수익률을 볼 수 있어요" — `caption` `textSecondary`, **차트 카드 안** 플롯 아래. 스크럽은 화면에 손잡이가 없는 제스처라 한 번은 말해 줘야 한다. 한 번이라도 스크럽했거나 데이터가 1건 이하면 감춘다. 같은 안내를 차트 `accessibilityHint` 가 이미 말하므로 이 줄은 `accessibilityHidden` 장식이다 |
...
벤치마크는 액세서리를 떠나 **툴바 버튼 하나 + 시트**가 된다:
...
액세서리는 대신 **기간·단위 스트립**이 된다(§3.1):

- **leading**: `YTD · 일별` + `chevron.up`. 스크롤로 YTD 큰 숫자가 가려지면 `연초 대비 +8.2%`
  로 교대하고, 스크럽 중에는 앞말이 `기간 시작 대비` 로 바뀐다
- **trailing**: 일별/월별 **전환 버튼 하나**. 라벨이 곧 현재 단위다(§3.1 순자산 통화 전환과
  같은 문법) — 누를 때마다 대상이 바뀌므로 선택 상태가 아니다
- **스트립 탭 → 기간 선택 시트**(1M/3M/6M/YTD/1Y/ALL)

근거: 성과 탭에서 반복되는 동작은 벤치마크를 겹쳤다 뗐다 하는 것이 아니라 **기간과 단위를
바꾸는 것**이다. 비교는 한 번 켜면 그대로 두지만 기간·단위는 차트를 볼 때마다 만진다. 즉시성
있는 둘을 액세서리로 올리고 저빈도인 벤치마크를 툴바로 내렸다. 확장할 대상(기간 시트)을 갖는
탭은 여전히 성과 하나라 §3.1 의 확장 규칙은 그대로 성립한다.
...
- 선택 벤치마크는 0~1개(단일 선택). **범례는 차트 카드 안**, 플롯 바로 위 한 줄이다 —
  ...
  "액세서리 leading 의 dot 이 범례" 는 액세서리가 기간·단위를 맡으면서 폐기)
...
- **빈도.** 기간·단위는 이 탭에서 가장 자주 만지는 둘이다. 인라인 컨트롤은 스크롤로 밀려
  ...
  액세서리는 늘 화면에 있다.
- **"컨트롤 하나" 규칙은 깨지지 않는다.** 액세서리 trailing 은 여전히 **단위 전환 하나**다.
...
- 셀을 누르면 **카드 안에서 한 줄이 펼쳐진다** — "8월 3일 · +1.2% · +₩142,000". 시트를 쓰지
  않는다: 이 탭엔 이미 기간·벤치마크 시트가 둘이라 셋째를 띄우면 "눌렀더니 또 시트"가 된다.
  같은 셀을 다시 누르면 접힌다
...
| TrendLineChart | 자산 추이·벤치마크 오버레이, area gradient, 스크럽, 0% 기준선·축 라벨, **범례 한 줄** | 단독(범례 없음) / 벤치마크 오버레이(범례 표시) / 데이터 1건 이하 안내 |
| CalendarHeatmap | 한 달치 일간 수익률 7열 히트맵 — 셀 44pt, 3단 채움 | 수익(gain 채움) / 손실(loss 채움 + 1pt 스트로크) / 변동 0%(neutral) / 기록 없음(채움 없음) |
...
| PeriodSegment | 기간 선택 1M~ALL 세그먼트 | **예약 — 현재 호출부 없음.** 성과 탭이 기간을 액세서리 + `PeriodPickerSheet` 로 옮기면서 인라인 자리가 사라졌다(§4.3). 인라인 기간 선택이 맞는 화면을 위해 컴포넌트는 남겨 둔다 |
...
| AccessoryControlButton | 액세서리 **상태 컨트롤** — 성과 일별/월별 전환 | 켜짐(brand 채움) / 꺼짐·중립(brand 1pt 스트로크) · 라벨형 / 아이콘형 · 선택 표시 있음/없음(전환형은 없음) |
