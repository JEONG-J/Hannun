...
| 차트 플롯 높이 240pt (PM-2) | §6.3 `Plot fRV0P` 는 338×320 | **320 → 240 으로 내렸다.** 320 은 "차트만 있는 탭"의 값이다. 그 뒤 차트 카드 안으로 범례 한 줄과 스크럽 가이드 한 줄이 들어왔고 카드 아래에 월간 캘린더가 붙어서, 320 을 유지하면 캘린더가 첫 화면에서 통째로 밀려난다. 240 은 X축 라벨 3개와 Y축 라벨 2개를 붙이고도 진폭이 읽히는 최소값이다 — §6.3 의 320 서술은 이후 대체됨 |
