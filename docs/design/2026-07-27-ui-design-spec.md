...
| ② 포트폴리오 | 총 평가금액+수익률 / "12종목 · 12:04 기준" (교대) | **입출금 기록** (스트로크 캡슐, `arrow.left.arrow.right`) | — | 보조 이동 진입 [PF-5/6] |
...
- 포트폴리오: **종목 추가가 액세서리에서 빠져 목록 화면 우상단 툴바 아이콘 1개로 옮겨간다**
  ...
  액세서리 trailing 은 대신 입출금 기록 하나다. 빈도가 낮은 이동이라 캡슐의 한 자리를
  내줘도 된다고 판단했다.
...
| ② 포트폴리오 | 요약 문장 + 입출금 기록 캡슐 | "₩8,420만" + 입출금 기록(아이콘 원형) |
...
- 하단 액세서리(§3.1): trailing 은 **입출금 기록**(스트로크 캡슐, `arrow.left.arrow.right`)
  → push [PF-5/6] 하나다. brand 채움이 아니라 스트로크인 이유는, 채움이 "여기서 무언가
  생성된다"는 신호인데 입출금 기록은 다른 화면으로 옮겨 갈 뿐 아무것도 만들지 않아서다.
...
| AccessoryActionButton | 액세서리 액션 버튼 (불투명) — **생성**(primary) / **보조 이동**(secondary) | primary(brand 채움 라벨 캡슐, 매매일지 작성) / secondary(brand 스트로크 캡슐, 포트폴리오 입출금 기록) · 둘 다 아이콘 원형 폴백 |
