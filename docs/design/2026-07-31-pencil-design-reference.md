...
- **TrendChartCard** `XQeWz` — surface, radius 16, padding 16.
  ...
  - 벤치마크 라인: `category-foreign`, 1pt, opacity 0.6 — **주선보다 확실히 약하게**.
    색은 **카테고리 원색**이 맞다(액세서리 dot 과 같은 토큰). 스펙 §4.3 에 한동안 "중립색"
    이라는 상충 서술이 있었으나 이 캔버스 기준으로 정리했다.
  - 축 라벨 3개(1월/4월/7월)를 `space_between` 으로 배치.
  - **0% 기준선과 Y축 라벨은 캔버스에 없지만 구현에는 넣었다** — §8 참고.
...
- 자동 날짜 "2026년 7월 27일 (월) 14:32" 13 secondary — 시안에서는 사용자가 입력하지 않는다.
- 제목 플레이스홀더 20 Semibold secondary → 본문 에디터 → 연결 종목 섹션.
- 연결 종목: "연결 종목 (선택)" 캡션 + `FilterChip` 다중 선택(선택 = brand 채움 + 흰 라벨).

> **구현은 이 화면을 `Form` 으로 바꿨고 날짜를 `DatePicker` 로 열었다** — §8 참고.
> 그 대가로 섹션 모서리 반경이 시스템 insetGrouped 값이 되어 여기 적힌 16 과 어긋난다.
...
| 스와이프 편집 | 정적 캔버스 한계 | 스펙 §6 모션 가이드. 단 포트폴리오 목록은 아래 사유로 `contextMenu` 를 쓴다 |
| 카테고리 `FilterChip` (PF-1) | §6.2 에 칩 줄이 없음 | NW-4(순자산 → 카테고리 딥링크) 진입점 때문에 필요. **필터가 걸린 동안에만** "전체 + 선택 카테고리" 두 칩을 목록 위에 고정하고, 필터를 풀면 줄째로 사라져 기본 화면은 시안과 같아진다 |
| 차트 0% 기준선 · Y축 라벨 (PM-2/PM-4) | §6.3 플롯에는 X축 라벨 3개뿐이고 가로선도 Y축 표기도 없음 | **둘 다 추가했다.** 시안은 전 구간 상승하는 대표 상태라 기준선이 플롯 바닥에 깔려 없어도 티가 나지 않지만, 실제 데이터는 0% 를 오르내린다 — 기준선이 없으면 내려간 구간이 손실인지 고점 대비 조정인지 구분되지 않는다. 기준선만 그으면 이번엔 진폭이 안 읽혀(벤치마크와 벌어진 간격이 1%p 인지 10%p 인지) 오버레이의 목적이 사라지므로 Y축 라벨을 trailing 에 붙였다. 처음엔 3개였는데 플롯을 240pt 로 내리면서 라벨끼리 붙어 **2개**로 줄였다(`desiredCount` 는 힌트라 값 범위에 따라 하나 더 그려질 수 있다 — 개수를 강제하지 않는다). 대신 **눈금선(grid)은 긋지 않아** 시안의 빈 플롯은 지킨다 — X축 라벨 3개, Y축 라벨 2개, 점선 하나가 전부다. 라벨 폭만큼 플롯 가로가 338 보다 줄어든다 |
| 스크럽 인디케이터 색 (PM-2) | 정적 캔버스라 스크럽 상태가 없음 | 초기 구현은 `separator` 였으나 흰 `surfacePrimary` 위에서 대비 약 1.3:1 이라 손가락이 짚은 자리가 보이지 않았다. 기준선과 같은 `neutral`(≈4.5:1) 로 올리고 **실선**으로 그어 가로·점선인 기준선과 구분한다 |
| 종목 행 수정/삭제 진입 (PF-3/4) | 정적 캔버스에 인터랙션 없음 | §6.2 카드 구조(헤더와 행이 카드 하나를 공유)는 `List` 의 섹션 모델로 만들 수 없어 `ScrollView` + 카드로 구현했다. 그 대가로 `swipeActions` 를 잃어 행 `contextMenu`(수정/삭제)가 대체 진입 경로다 |
| 행 사이 구분선 (PF-1) | §6.2 는 구분선을 "좌우 여백에서 끊는 구조"라 서술하지만 캔버스에는 선이 그려져 있지 않음 | 서술을 따라 `Divider` 를 padding `[0, 16]` 으로 넣었다 — §6.7 입출금 카드와 같은 규칙이다. 캔버스대로 선을 빼려면 `PortfolioCategoryCard` 의 `Divider` 한 줄만 지우면 된다 |
| 입출금 기록의 뒤로 버튼 (PF-5/6) | §6.7 compact 바가 `뒤로 110 ｜ 타이틀 ｜ 액션 110` 3분할이라 왼쪽에 뒤로가 있음 | **뒤로를 두지 않았다.** 이 화면은 push 된 하위 화면이 아니라 포트폴리오 탭의 **다른 한 면**이고(UI 스펙 §3.1), 오가는 자리는 하단 액세서리 캡슐 하나다. 뒤로를 남기면 돌아오는 길이 둘로 갈리는 데다, 캡슐은 push 위에도 떠 있어 같은 화면을 두 겹으로 쌓게 된다. 좌우 폭 고정으로 타이틀을 중앙에 두는 §6.7 규칙은 SwiftUI `.inline` 이 알아서 지킨다 |
| 종목 검색·정렬·필터 (PF-1) | §6.2 내비게이션 바에 검색·정렬·필터 컨트롤이 없음 | 종목 수가 늘면 카테고리 필터만으로는 한 종목에 닿을 수 없어 추가했다. 검색은 `searchable` + `searchToolbarBehavior(.minimize)` 로 **오른쪽 위 돋보기 버튼**에 접어 두어 콘텐츠 열은 시안 그대로 남는다. 왼쪽 위 `Menu`(`line.3.horizontal.decrease.circle`)는 정렬(평가금액순/수익률순/가나다순)과 카테고리 필터(다중 선택 `Toggle`)를 함께 담는다 — 둘 다 새 화면을 열지 않고 눈앞의 목록만 조용히 손보는 조작이라 한 메뉴로 묶었다. 정렬은 카드 **안** 행 순서만 바꾼다 — 카테고리 카드 순서는 순자산 도넛 섹터와 같아야 하므로 고정이다. 정렬이 기본값을 벗어났거나 필터가 걸리면 아이콘이 `.fill` 로 바뀐다 |
| 일지 검색·종목 필터 위치 (JR-1/JR-4) | §6.4 는 검색 바와 필터 칩 줄을 **목록 위 고정 행**으로 그렸다 | 그대로 두면 스크롤이 그 줄 **아래에서만** 일어나 Large Title 이 걷히지 않는다. 둘 다 툴바로 올렸다 — 검색은 §6.2 와 같은 `searchable` + `.minimize`(우), 종목 필터는 `Menu` + inline `Picker`(좌). 칩 줄이 사라져 "지금 걸러져 있다"를 말할 자리를 잃으므로, 필터가 걸리면 아이콘을 `.fill` 로 바꾸고 고른 종목 이름은 `accessibilityValue` 로 읽어 준다 |
| 일지 목록 상태 문구 위치 (JR-1) | §6.4 액세서리 leading 에 "이번 달 7건" | `navigationSubtitle` 로 올렸다. 목록의 범위를 말하는 문장은 제목의 일부라 화면 반대쪽 끝에 있으면 제목과 눈이 두 번 오간다. 액세서리에 남는 동작이 작성 하나뿐이 되면서 **캡슐째 버튼**이 됐다 — 오른쪽 알약 하나만 눌리게 두면 좁은 과녁을 위해 캡슐 나머지가 죽는다 |
| 태그 캡슐 분류 구분 (JR-1/JR-3) | §6.4 셀의 `TagPill` 이 전부 같은 중립 tint | 이름을 읽기 전에는 무슨 자산에 달린 기록인지 구분되지 않는다. 표시용 태그에 **분류색 `.clear` 유리**(`GlassRole.categoryTag(_:)`)를 입혔다 — 카드 **안**에 놓이는 캡슐이라 뒤를 흐리는 `.regular` 는 맞지 않는다. 작성 폼의 선택형 칩은 폼 안이라 유리를 못 쓰므로(스펙 §5) **같은 분류색을 불투명 wash 로** 깐다: 재질만 갈리고 색은 하나다. "+n" 은 색을 입지 않는다 — 가려진 태그들의 분류가 하나가 아니다 |
| 작성 시각 입력 UI (JR-2) | §6.8 의 날짜는 읽기 전용 13 secondary 한 줄 | 매매일지는 사후에 몰아 쓰는 일이 잦아 자동 기록이면 날짜가 매매일이 아니라 "쓴 날"이 된다 — 정렬·"오늘/어제"·"이번 달 n건" 이 전부 이 값을 본다. `DatePicker(.compact)`(날짜 + 시:분)로 열고 기본값·상한을 "지금"으로 뒀다. 저장 시각(`updatedAt`)은 따로 남는다 |
| 일지 작성 폼 골격 (JR-2) | §6.8 은 필드를 세로로 그냥 쌓았고 카드 radius 16 | 어디까지가 한 덩어리인지가 배치로 드러나지 않아 `Form` 3섹션(작성 시각 / 내용 / 연결 종목)으로 끊었다. 섹션 모서리는 시스템 insetGrouped 값이라 **16 과 어긋난다** — 그 하나 때문에 리스트를 직접 그리면 폼이 주는 키보드·포커스 처리가 통째로 날아가서 시스템 값을 받아들였다. 배경(`surfacePrimary`)과 헤더 타이포(`textCase(nil)` + 토큰 서체)는 덮어썼다 |
| 차트 플롯 높이 240pt (PM-2) | §6.3 `TrendChartCard XQeWz` 의 플롯 338×320 | **320 → 240 으로 내렸다.** 320 은 "차트만 있는 탭"의 값이다. 그 뒤 차트 카드 안으로 범례 한 줄과 스크럽 가이드 한 줄이 들어왔고 카드 아래에 월간 캘린더가 붙어서, 320 을 유지하면 캘린더가 첫 화면에서 통째로 밀려난다. 240 은 X축 라벨 3개와 Y축 라벨 2개를 붙이고도 진폭이 읽히는 최소값이다 — §6.3 의 320 서술은 이후 대체됨 |
| 기간·단위 컨트롤 위치 (PM-2) | §6.3 `ControlsRow cdibb` 가 차트 바로 아래 인라인 | **둘을 갈랐다.** 기간 6칸은 캡슐 폭에 못 들어가 액세서리 leading 탭 → `PeriodPickerSheet` 로 뺐다 — 캘린더가 붙으면서 인라인 줄이 화면 밖으로 밀리기 때문이다. 단위(일별/월별)는 한 번 액세서리 trailing 전환 버튼으로 올렸다가 **차트 카드 헤더의 2칸 세그먼트로 되돌렸다**: 차트 가로축만 바꾸는 컨트롤이라 바꾸는 대상 바로 위에 있어야 하고, 2칸 배타 선택은 "현재 상태를 라벨로 적는" 전환 버튼보다 세그먼트가 정직하다. 시안과 달리 플롯 **아래**가 아니라 **위** 한 줄이며 범례와 자리를 나눠 쓴다 — 근거는 `2026-07-27-ui-design-spec.md` §4.3 |
| 벤치마크 컨트롤 위치 (PM-4) | §7 표가 액세서리 `EikF0` 의 칩 4개를 "현재 벤치마크 1개 + 메뉴 전개"로 축약한다고 서술 | 지수를 **고르는** 일은 저빈도라 **툴바 버튼 하나 + 시트**로 내려갔다. 시트 첫 섹션이 비교 on/off 스위치, 아래가 지수 4행이다. 반대로 겹쳤다 뗐다 하는 **비교 on/off 는 고빈도**라 액세서리 trailing 에 남겼다 — 시트 안에 두면 한 번 끄는 데 탭 세 번이 든다. §7 의 성과 행 축약 규칙(칩 4개 → 1개 + 메뉴)은 이후 대체됨 |
| 차트 카드 안 범례 (PM-4) | 시안에 범례 자리가 없음 — 칩의 선택색이 곧 범례였음 | 칩을 없앤 뒤 한동안 **액세서리 leading 의 dot** 이 범례였는데, 액세서리 leading 이 기간·수익률 문구로 바뀌면서 그 자리마저 사라졌다. 범례는 플롯 바로 위 한 줄(카테고리 dot + 지수 이름, `caption`/`textSecondary`)로 차트 카드 안에 넣었고, **같은 줄 오른쪽 끝을 일별/월별 세그먼트가 쓴다**. **겹쳐진 선이 있을 때만** 그린다. 내 라인은 넣지 않는다 — 화면에서 유일한 굵은 brand 선이라 헷갈릴 대상이 없다 |
| 스크럽 가이드 한 줄 (PM-2) | 정적 캔버스라 제스처 안내가 없음 | 스크럽은 화면에 손잡이가 없어 그런 게 있는 줄 모르면 영영 안 쓴다. 플롯 아래 `caption`/`textSecondary` 한 줄로 "차트를 좌우로 쓸어 시점별 수익률을 볼 수 있어요"를 두고, **한 번이라도 스크럽했거나 데이터가 1건 이하면 감춘다**. 같은 안내를 차트 `accessibilityHint` 가 이미 말하므로 이 줄은 `accessibilityHidden` 장식이다 |
| 월간 수익률 캘린더 (PM-4) | 시안 §6.3 에 아예 없음 — 성과 탭은 히어로·차트·컨트롤 세 덩어리가 전부 | 차트는 "얼마나 올랐나"만 답한다. "**어떤 날에** 올랐나"를 답할 화면이 없어 차트 카드 아래 인라인 카드로 추가했다. 7열 격자·셀 44pt·3단 채움이며, 셀을 누르면 시트가 아니라 카드 안 한 줄이 펼쳐진다(이 탭엔 이미 시트가 둘이다). 고른 칸에는 `brand` 2pt 링이 얹히고 히어로·차트 커서도 그 시점으로 함께 옮겨 간다 — 캘린더에서 고른 날이 차트와 다른 곳을 가리키면 두 영역이 서로 다른 화면처럼 읽힌다. 달 이동은 ◀▶ 외에 **헤더 라벨을 눌러 펼치는 월 점프 격자**(3열 × 4행 + 년 이동)가 있다: 작년 3월에 닿으려고 ◀ 를 열여섯 번 누르지 않게 한다. 시안에 대응 노드가 없으므로 카드 문법은 §6.3 차트 카드를 그대로 따랐다 |
