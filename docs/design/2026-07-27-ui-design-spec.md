...
|----|------|------|------|------|
| ① 순자산 | 총자산 / "**12:04** 시세 기준" (교대) | 통화 전환 (스트로크 캡슐) | — | 시세 신선도 + 통화 전환 [NW-2] |
| ② 포트폴리오 | 총 평가금액+수익률 / "12종목 · 12:04 기준" (교대) | **종목 추가** (brand 채움, `plus`) | — | 생성 진입 [PF-5/6] |
...
- 순자산: 통화 토글이 액세서리로 이동하므로 **큰 숫자 옆의 CurrencyToggle은 제거**한다(§4.1).
  액세서리의 통화 컨트롤은 2칸 세그먼트가 아니라 **전환 대상 통화 코드를 라벨로 쓴 버튼 하나**다
  (현재 KRW → `USD`). 현재 통화는 leading 의 통화 기호가 말한다. 수동 갱신은 pull-to-refresh 몫이다.
- 포트폴리오: **입출금 기록은 액세서리에서 빠져 툴바 아이콘 1개로 돌아간다** [PF-2].
  액세서리 trailing 은 종목 추가 하나다.
...
|----|-------------|------------------|
| ① 순자산 | 총자산/갱신 시각 + 통화 전환 캡슐 | "₩1억 2,340만" + 통화 전환 캡슐(라벨 유지) |
| ② 포트폴리오 | 요약 문장 + 종목 추가 캡슐 | "₩8,420만" + 종목 추가(아이콘 원형) |
...
- 하단 액세서리(§3.1): trailing 은 **종목 추가**(brand 채움) → 단계식 sheet [PF-2] 하나다.
  leading 은 요약 바가 보이는 동안 "12종목 · 12:04 기준", 가려지면 총 평가금액+수익률로 교대
- **입출금 기록은 툴바 아이콘 1개**(`arrow.left.arrow.right`) → push [PF-5/6].
  액세서리 trailing 이 컨트롤 하나로 제한되면서 이 액션이 밖으로 나왔다. 액세서리와 툴바
  양쪽에 두는 중복 배치는 여전히 금지다
- **카테고리 필터는 액세서리에 넣지 않는다.** 칩 그룹 자체가 §3.1 금지 대상인 데다,
  캡슐 가용 폭 358pt를 5분할하면 약 71pt로 한글 라벨(국내주식·해외주식)과 44pt 터치 타깃을
  동시에 만족할 수 없다. 필터는 요약 바 아래 가로 스크롤 ChipGroup으로 둔다
...
| CurrencyToggle | KRW/USD 전환 세그먼트 — **입출금 폼 전용**(액세서리에서는 전환 버튼 사용) | KRW 선택 / USD 선택 |
...
| AccessoryActionButton | 액세서리 **생성 액션** 버튼 (불투명) | primary(brand 채움 라벨 캡슐) / 아이콘 44pt 원형 · secondary 는 액세서리 밖 용도 |
| AccessoryControlButton | 액세서리 **상태 컨트롤** — 통화 전환·비교 on/off | 켜짐(brand 채움) / 꺼짐·중립(brand 1pt 스트로크) · 라벨형 / 아이콘형 |
