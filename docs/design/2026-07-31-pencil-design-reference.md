...
| 기간·단위 컨트롤 위치 (PM-2) | §6.3 `ControlsRow cdibb` 가 차트 바로 아래 인라인 | **인라인 줄을 없애고 액세서리 + 시트로 옮겼다.** 캘린더가 붙으면서 그 아래로 스크롤할 일이 생겨 인라인 컨트롤이 화면 밖으로 밀린다. 단위(일별/월별)는 액세서리 trailing 전환 버튼 하나, 기간 6칸은 캡슐 폭에 못 들어가 leading 탭 → `PeriodPickerSheet` 로 뺐다. 시안의 `ControlsRow cdibb` 는 이후 대체됨 — 근거는 `2026-07-27-ui-design-spec.md` §4.3 |
| 벤치마크 컨트롤 위치 (PM-4) | §7 표가 액세서리 `EikF0` 의 칩 4개를 "현재 벤치마크 1개 + 메뉴 전개"로 축약한다고 서술 | 액세서리가 고빈도인 기간·단위를 맡으면서 **벤치마크는 툴바 버튼 하나 + 시트**로 내려갔다. 시트 첫 섹션이 비교 on/off 스위치, 아래가 지수 4행이다. §7 의 성과 행 축약 규칙(칩 4개 → 1개 + 메뉴)은 이후 대체됨 |
| 차트 카드 안 범례 (PM-4) | 시안에 범례 자리가 없음 — 칩의 선택색이 곧 범례였음 | 칩을 없앤 뒤 한동안 **액세서리 leading 의 dot** 이 범례였는데, 액세서리가 기간·단위로 바뀌면서 그 자리마저 사라졌다. 범례는 플롯 바로 위 한 줄(카테고리 dot + 지수 이름, `caption`/`textSecondary`)로 차트 카드 안에 넣었다. **겹쳐진 선이 있을 때만** 그린다. 내 라인은 넣지 않는다 — 화면에서 유일한 굵은 brand 선이라 헷갈릴 대상이 없다 |
...
| 월간 수익률 캘린더 (PM-4) | 시안 §6.3 에 아예 없음 — 성과 탭은 히어로·차트·컨트롤 세 덩어리가 전부 | 차트는 "얼마나 올랐나"만 답한다. "**어떤 날에** 올랐나"를 답할 화면이 없어 차트 카드 아래 인라인 카드로 추가했다. 7열 격자·셀 44pt·3단 채움이며, 셀을 누르면 시트가 아니라 카드 안 한 줄이 펼쳐진다(이 탭엔 이미 시트가 둘이다). 시안에 대응 노드가 없으므로 카드 문법은 §6.3 차트 카드를 그대로 따랐다 |
