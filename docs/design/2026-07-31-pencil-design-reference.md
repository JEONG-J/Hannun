...
| 종목 검색·정렬 (PF-1) | §6.2 내비게이션 바에 검색·정렬 컨트롤이 없음 | 종목 수가 늘면 카테고리 필터만으로는 한 종목에 닿을 수 없어 추가했다. 검색은 `searchable` + `searchToolbarBehavior(.minimize)` 로 **오른쪽 위 돋보기 버튼**에 접어 두어 콘텐츠 열은 시안 그대로 남는다. 정렬(평가금액순/수익률순/가나다순)은 왼쪽 위 `Menu` 이며 카드 **안** 행 순서만 바꾼다 — 카테고리 카드 순서는 순자산 도넛 섹터와 같아야 하므로 고정이다 |
