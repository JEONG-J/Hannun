...
/// KRW / USD 전환 세그먼트. **입력 폼 전용**이다 — 종목 편집 화면에서 이 종목이 어느 통화로
/// 굴러가는지 정할 때 쓴다.
///
/// 입출금 폼은 이걸 쓰지 않는다. 그쪽 통화는 독립된 값이 아니라 방금 적는 숫자의 단위라, 금액
/// 행 안의 단위 자리를 그대로 메뉴로 만들어 눈을 옮기지 않고 바꾸게 했다. 종목 폼의 통화는
/// 붙을 숫자가 아직 없는 단계에서 고르므로 접을 이유가 없고, 그래서 값 둘을 펼쳐 둔다.
///
