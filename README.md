...
그때그때 계산합니다. 서버 없이 **SwiftData + CloudKit** 으로 여러 아이폰이 같은 데이터를 봅니다.
...
> 하단 캡슐은 iOS 26 `tabViewBottomAccessory` 입니다. 순자산·포트폴리오·투자 성과는 캡슐 전체가
> 버튼이고(통화 전환 · 입출금 기록 · %↔₩ 전환), 매매일지만 오른쪽에 작성 버튼을 따로 답니다.
