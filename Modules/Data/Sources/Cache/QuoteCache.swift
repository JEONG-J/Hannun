...
/// 시세 캐시. KIS·업비트 모두 호출량 제한이 있어 §11.1·§11.2 가 15분 캐싱을 요구한다.
///
...
actor QuoteCache {
    ...
    /// - Parameters:
    ///   - timeToLive: 유효 기간. 기본 15분 (§11.1·§11.2)
    ...
    /// 유효 기간 안의 값만 돌려준다.
    ///
    /// 만료된 항목을 **지우지 않는다** — §8 의 폴백은 갱신 실패 시점에 만료된 값을 다시 꺼내 쓰기
    ...
    /// §8 의 "갱신 실패 시 마지막 캐시값 사용 + 갱신 실패 배지" 정책에 쓰인다.
